---
title: "Exploratory Data Analysis for Areal Data"
author: "Sudipto Banerjee"
date: "October 15, 2025"
output: html_document
---

```{r clear memory, include=TRUE}
rm(list=ls())
```
...
1. Read the shapefile and the data set. Create an "nb" data structure from the polygon to access neighbors of regions on the map. 
```{r}
...
2. Access neighbors of each region.
```{r}
...
3. Create a map of raw SIDS rates. First create a vector of rates from counts
```{r}
nc.sids.rates.raw = probmap(nc.sids$SID79, nc.sids$BIR79)$raw
```

4. Alternatvely, one may want to use Freeman-Tukey transformations
```{r}
nc.sids.rates.FT = sqrt(1000) * (sqrt(nc.sids$SID79/nc.sids$BIR79) + sqrt((nc.sids$SID79 + 1)/nc.sids$BIR79))
```

5. Append the rates we want to plot to the nc.sids data frame
```{r}
nc.sids$rates.FT = nc.sids.rates.FT 
brks = c(0, 2.0, 3.0, 3.5, 6.0)
```

6. Compute Moran's I and Geary's C
```{r}
...
print(nc.sids.moran.out)
nc.sids.geary.out = geary.test(nc.sids.rates.FT, listw=ncCC89.listw, zero.policy=TRUE)
print(nc.sids.geary.out)
```

7. SAR model regressing rates.FT on NWBIR79.FT. Begin with a Freeman-Tukey transformation on non-white birth rates
```{r}
...
9. CAR model regressing rates.FT on NWBIR79.FT
```{r}
...
10. Draw the maps using the spplot (trellis) graphics function
```{r}
...
11. Draw the maps using the maps function
```{r}
library(maps)
library(classInt)
...
class.fitted = classIntervals(var=nc.sids$fitted.car, n=4, style="fixed", fixedBreaks=brks, dataPrecision=4)
color.code.fitted = findColours(class.fitted, color.pallete)

leg.txt = c("<2.0", "2.0-3.0", "3.0-3.5",">3.5")

#pdf(file="SidsCar.pdf")
...
plot(nc.sids[c("geometry","fitted.car")], col=color.code.fitted, main = "", key.pos = NULL, reset = FALSE)
