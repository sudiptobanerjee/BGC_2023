...
In this section, JAGS receives the prior precision $M_0^{-1}$, prior mean $\mu_0$, pre-sampled $\tau$, and $M$ replicated rows of $Y$ (`Y_rep`). JAGS computes the conditional likelihood $Y_{\text{rep}} \sim \mathcal{N}(X\beta, \tau^{-1})$ internally, using its built-in conjugate engine to draw exact posterior samples in 1 iteration without adaptation (`n.adapt = 0`).
