...
In this laboratory, we compare the pure execution speed of exact analytical sampling methods for Bayesian Linear Regression. To ensure a fair comparison and avoid memory limits, we explicitly separate the heavy C++ compilation phase from the raw sampling execution phase.

## 2. Setup and Data Loading

We load the libraries, strictly limit processing to 1 core to prevent memory spikes, and prepare our data and marginal distributions.
...
## 3. Phase 1: Pre-Compilation (Untimed)

We pull the inline models directly from the wrapper source code and compile them into C++ pointers upfront. This takes the time and memory hit before the benchmarking begins.

```{r compilation, message=FALSE, warning=FALSE, results='hide'}
...
## 4. Phase 2: Execution Benchmarks

Now we time the pure sampling execution. JAGS does not require heavy C++ compilation, so we can safely time its direct wrapper. 
